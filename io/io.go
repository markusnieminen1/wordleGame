...
func AppendToCSV(fullpath string, Obj GameResultObject) {
	...
	*/
	...
	if fileinfo.Size() < 1 {
		...
		file_columns := `username,secret word,number of attempts,game won,start time,elapsed time`
	...
	comma := ","
	byte_arr := StrToByteArr(comma + string(byte(10)) + Obj.User + comma + Obj.SecretWord + comma + fmt.Sprint(Obj.Attempts) + comma + fmt.Sprint(Obj.WinLose) + comma + fmt.Sprint(Obj.Stime) + comma + fmt.Sprint(Obj.Elapsed))
